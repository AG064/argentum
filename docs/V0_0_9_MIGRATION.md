...
| File permission policy and approvals | Migrated | Workspace path containment, project-scoped Read Only, Confirm Before Changes, exact custom read/write policy, write approval, approve, and reject paths exist. Policy is enforced at model advertisement, manual request, approval resume, and execution. Session-scoped grants and restart recovery remain pending. |
