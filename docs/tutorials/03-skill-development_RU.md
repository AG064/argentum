...
В этом уроке вы создадите собственный навык, который добавит новые возможности вашему агенту. Навыки (skills) — это основной способ расширения Argentum: они регистрируют инструменты, которые агент может вызывать во время разговоров.
...
1. Добавьте его в репозиторий Argentum через pull request
...
| Инструмент всегда возвращает ошибку | Проверьте `argentum gateway logs` на наличие реального исключения |
| Функция не загружается | Запустите `argentum doctor` для диагностики проблем с зависимостями |
