...
function expandPath(p) {
    if (p.startsWith('~/') || p === '~') {
        return (0, path_1.resolve)((0, os_1.homedir)(), p.slice(2));
    }
...
class SandboxExecutor {
    ...
    constructor(config = {}) {
        ...
        this.logger = (0, logger_1.createLogger)().child({ feature: 'sandbox-executor' });
     ...
     */
    async execute(code, lang, options) {
...
// ─── Singleton ────────────────────────────────────────────────────────────────
let instance = null;
function getSandboxExecutor(config) {
    if (!instance) {
