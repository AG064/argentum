...
class ErrorAnalyzer {
    ...
    memoryDir;
    lessonsPath;
    constructor(sessionsDbPath, memoryDir) {
    ...
    async findUserCorrections() {
        ...
        try {
            ...
            const messages = db
                .prepare(`SELECT m.*, s.title as session_title
         FROM messages m
    ...
    async findFailedTasks() {
        const analyses = [];
        if (!(0, fs_1.existsSync)(this.lessonsPath)) {
            return analyses;
        }
        try {
            const content = fs.readFileSync(this.lessonsPath, 'utf8');
            const failedMatches = content.matchAll(/## FAILED[^\n]*\n([^\n]*\n){0,3}?([^\n]*error[^\n]*|failed|wrong|incorrect)/gi);
     ...
     */
    inferRootCause(pattern, _corrections) {
        const causes = {
     ...
     */
    extractLessons(pattern, _corrections) {
        const lessons = [];
        switch (pattern) {
            case 'output_formatting':
     ...
     */
    suggestFix(pattern, _corrections) {
        const fixes = {
