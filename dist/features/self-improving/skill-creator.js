...
class SkillCreator {
    ...
    memoryDir;
    threshold;
    constructor(skillsDir, memoryDir, threshold = 5) {
     ...
     */
    async createSkills(complexTasks) {
        const created = [];
        // Filter by threshold
     ...
     */
    createNewSkill(template) {
        const skillPath = path.join(this.skillsDir, template.name);
        if (!(0, fs_1.existsSync)(skillPath)) {
            (0, fs_1.mkdirSync)(skillPath, { recursive: true });
        }
        // Create SKILL.md
        ...
        (0, fs_1.mkdirSync)(path.join(skillPath, 'references'), { recursive: true });
        // Create scripts directory
        (0, fs_1.mkdirSync)(path.join(skillPath, 'scripts'), { recursive: true });
    }
    /**
     * Update an existing skill
     */
    updateExistingSkill(skillPath, template) {
        ...
        if ((0, fs_1.existsSync)(skillMdPath)) {
            ...
            if (!existing.includes(template.description)) {
                const updated = existing.replace(/(#+\s*Usage\s*\n)([\s\S]*?)(?=\n#|$)/, (_, header, existingUsage) => {
...
`;
     ...
     */
    async detectPatterns(sessionsContent) {
        const templates = [];
