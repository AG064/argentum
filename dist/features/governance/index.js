...
class GovernanceFeature {
    ...
    async reject(ticketId, approver, reason) {
        ...
        const now = Date.now();
        this.db
            .prepare('UPDATE approval_tickets SET status = ?, approver = ?, reason = ?, resolved_at = ? WHERE id = ?')
    ...
    createTicket(action, requester, status, reason) {
        ...
        this.db
            .prepare(`INSERT INTO approval_tickets
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(id, action.type, action.description, JSON.stringify(action.payload), action.risk, requester, status, status === 'approved' ? 'system' : null, reason ?? null, now, status === 'approved' ? now : null, expiresAt);
        if (status === 'pending') {
    ...
    getExpiredCount() {
        const now = Date.now();
        const row = this.db
            .prepare("SELECT COUNT(*) as c FROM approval_tickets WHERE status = 'pending' AND expires_at <= ?")
            .get(now);
        return row.c;
    }
    expireOldTickets() {
        ...
        const result = this.db
            .prepare("UPDATE approval_tickets SET status = 'expired', resolved_at = ? WHERE status = 'pending' AND expires_at <= ?")
            .run(now, now);
        if (result.changes > 0) {
