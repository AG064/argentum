...
class LifeDomainsFeature {
    ...
    updateDomain(id, updates) {
        ...
        this.db
            .prepare('UPDATE domains SET name=?, description=?, color=?, icon=?, priority=?, updated_at=? WHERE id=?')
    ...
    addEntry(domainId, type, title, content, options) {
        ...
        this.db
            .prepare(`INSERT INTO domain_entries (id, domain_id, type, title, content, tags, importance, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    ...
    updateEntry(id, updates) {
        ...
        const merged = { ...existing, ...updates, updatedAt: Date.now() };
        this.db
            .prepare('UPDATE domain_entries SET title=?, content=?, tags=?, importance=?, metadata=?, updated_at=? WHERE id=?')
