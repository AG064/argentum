...
class SlackIntegrationFeature {
    ...
    async init(config, context) {
        ...
        this.config = {
            ...
            secretEnvVar: config['secretEnvVar'] ?? this.config['secretEnvVar'],
        };
        this.initDatabase();
        // Load persisted config
        const cfg = this.db.prepare('SELECT * FROM config WHERE key = ?').get('slack');
        if (cfg) {
            try {
    ...
    async healthCheck() {
        try {
            const sent = this.db.prepare('SELECT COUNT(*) as c FROM sent_messages').get().c;
            const received = this.db.prepare('SELECT COUNT(*) as c FROM received_events').get().c;
            return {
                healthy: true,
                details: {
     ...
     */
    configure(botToken, signingSecret, appToken, socketMode = false) {
     ...
     */
    async sendEphemeral(channel, user, _text, _blocks) {
        // Stub implementation
     ...
     */
    verifySignature(_signingSecret, _timestamp, _signature, _body) {
    ...
    getStats() {
        const sent = this.db.prepare('SELECT COUNT(*) as c FROM sent_messages').get()
            .c;
        const received = this.db.prepare('SELECT COUNT(*) as c FROM received_events').get().c;
        const interactions = this.db.prepare('SELECT COUNT(*) as c FROM received_interactions').get().c;
        return {
    ...
    /** Log received event */
    logReceivedEvent(payload) {
        this.db
            .prepare(`
      INSERT INTO received_events (event_id, event_type, team_id, channel, user, text, ts, received_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `)
            .run(payload.eventId, payload.event.type, payload.teamId, payload.event.channel ?? null, payload.event.user ?? null, payload.event.text ?? null, payload.event.ts, Date.now());
    }
    /** Log received interaction */
    logReceivedInteraction(interaction) {
        this.db
            .prepare(`
...
exports.default = new SlackIntegrationFeature();
