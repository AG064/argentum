...
exports.preReasoningHook = preReasoningHook;
const GOAL_PATTERNS = [
    /goal[:\s]+(.+)/i,
    /intention[:\s]+(.+)/i,
    /objective[:\s]+(.+)/i,
 ...
 */
function extractGoal(toolResults) {
 ...
 */
function extractDecisions(toolResults) {
    const decisions = [];
    for (const result of toolResults) {
 ...
 */
function inferNextSteps(toolResults) {
    for (const result of toolResults) {
 ...
 */
function extractCritical(toolResults) {
    const critical = [];
    for (const result of toolResults) {
 ...
 */
function compactProgress(toolResults, maxTokens) {
 ...
 */
function preReasoningHook(toolResults, maxTokens = 500) {
    return {
...
//# sourceMappingURL=pre-reasoning-hook.js.map