...
exports.HierarchicalMemoryStore = exports.MemoryTier = void 0;
const crypto_1 = require("crypto");
const fs_1 = require("fs");
const path_1 = require("path");
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const logger_1 = require("./logger");
const log = (0, logger_1.featureLogger)('hierarchical-memory');
// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------
var MemoryTier;
...
// ---------------------------------------------------------------------------
function generateId() {
    return (0, crypto_1.createHash)('sha256').update(Date.now().toString() + Math.random().toString()).digest('hex').slice(0, 16);
}
function serializeTags(tags) {
    return JSON.stringify(tags);
}
function deserializeTags(raw) {
    try {
...
function contentSimilarity(a, b) {
    const normalize = (s) => s.toLowerCase().split(/\s+/).filter((w) => w.length > 2);
...
class HierarchicalMemoryStore {
    ...
    flushPending = false;
    constructor(dbPath) {
        const resolved = (0, path_1.resolve)(dbPath ?? './data/hierarchical-memory.db');
    ...
    `).all(MemoryTier.LONG, DEMOTE_IMPORTANCE);
    ...
    // -------------------------------------------------------------------------
    compact() {
        for (const tier of [MemoryTier.SHORT, MemoryTier.MID, MemoryTier.LONG]) {
            const rows = this.db.prepare(`SELECT * FROM memory_entries WHERE tier = ? ORDER BY created_at DESC`).all(tier);
            const merged = new Set();
            for (let i = 0; i < rows.length; i++) {
                if (merged.has(rows[i].id))
                    continue;
                for (let j = i + 1; j < rows.length; j++) {
                    if (merged.has(rows[j].id))
                        continue;
    ...
    enforceShortCap() {
        ...
        const { count } = countStmt.get(MemoryTier.SHORT);
        if (count > SHORT_MAX) {
    ...
    countTier(tier) {
        const { count } = this.db.prepare(`SELECT COUNT(*) as count FROM memory_entries WHERE tier = ?`).get(tier);
        return count;
    }
    rowToEntry(row) {
        return {
...
exports.HierarchicalMemoryStore = HierarchicalMemoryStore;
