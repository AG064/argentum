...
exports.ModelRouter = exports.DEFAULT_SCORING_WEIGHTS = void 0;
...
class ModelRouter {
    ...
    defaultWeights;
    constructor(config) {
        this.defaultWeights = config?.defaultWeights ?? exports.DEFAULT_SCORING_WEIGHTS;
    ...
    scoreModel(model, criteria) {
        ...
        // 8. Specialization Match
        const specializationScore = this.scoreSpecializationMatch(model.specialization ?? [], criteria.taskType);
        // 9. Reliability Score
        ...
        const contextAvailabilityScore = this.scoreContextAvailability(model.contextLength ?? 32000, criteria.minContextLength);
        ...
        const totalScore = costScore * weights.costEfficiency +
    ...
    scoreLatency(latency, maxLatency) {
    ...
    scoreCapabilityMatch(capabilities, criteria) {
        let score = 0.5; // base
        ...
        if (criteria.preferredCapabilities?.length) {
            const matchCount = criteria.preferredCapabilities.filter(cap => capabilities.includes(cap)).length;
            score += (matchCount / criteria.preferredCapabilities.length) * 0.2;
        }
        return Math.min(1, score);
    }
    scoreContextLengthFit(contextLength, minRequired) {
        ...
        if (minRequired && contextLength < minRequired)
            return 0;
        if (!minRequired)
            return 0.7; // neutral
    ...
    scoreToolSupport(hasTools, required) {
        if (required && !hasTools)
            return 0;
        return hasTools ? 1 : 0.3;
    }
    scoreSuccessRate(model) {
        if (model.totalRequests === 0)
            return 0.7; // neutral for new models
    ...
    scoreTokenEfficiency(costPer1K) {
        // Similar to cost but more aggressive scaling
        return Math.max(0, 1 - costPer1K / 10);
    }
    scoreSpecializationMatch(specializations, taskType) {
        if (!taskType || specializations.length === 0)
            return 0.5;
    ...
    scoreFreshness(model) {
        // Prefer models that haven't been used in a while
        if (!model.lastSuccessAt && !model.lastFailureAt)
            return 0.7;
    ...
    getEligibleModels(criteria) {
        return Array.from(this.models.values()).filter(model => {
            ...
            if (criteria.requiredCapabilities?.length) {
                const hasAll = criteria.requiredCapabilities.every(cap => model.capabilities.includes(cap));
                if (!hasAll)
                    return false;
            }
            ...
            if (criteria.minContextLength) {
                ...
                if (ctxLen < criteria.minContextLength)
                    return false;
            }
            return true;
        });
