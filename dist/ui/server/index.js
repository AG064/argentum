...
function firstEnvOrDefault(names, fallback) {
...
function loadConfig() {
    ...
    if (fs.existsSync(configPath)) {
        try {
            ...
            console.warn('[Dashboard Server] Failed to parse Argentum config:', err.message);
        }
    }
        ...
        if (!envPass) {
            ...
            if (!loaded) {
                ...
                try {
                    ...
                    catch { /* chmod may fail on some platforms */ }
                }
                catch {
 ...
 * Serve static files
 */
function serveStatic(filePath, res, mimeType) {
...
function handleRequest(req, res) {
    ...
    if (auth?.username !== config.auth.username ||
...
function handleBudgetAPI(req, res, body) {
...
function handleOrgChartAPI(req, res, body) {
    sendJSON(res, 200, {
        tree: {
            name: 'CEO',
            status: 'active',
...
function handleOrgAPI(req, res, pathname, body) {
...
function handleSelfImprovingAPI(req, res, body) {
    sendJSON(res, 200, {
        enabled: true,
...
function handleTrajectoryAPI(req, res, body) {
    sendJSON(res, 200, {
        sessions: [
...
function handleSkillsAPI(req, res, body) {
    sendJSON(res, 200, {
        installed: [
            {
                name: 'clawhub',
    ...
    return new Promise((resolve, reject) => {
        ...
        server.on('error', (err) => {
            if (err.code === 'EADDRINUSE') {
                ...
                console.error('[Dashboard Server] Try a different port: argentum dashboard start --port 3001');
            }
            reject(err);
        });
        // Start listening
