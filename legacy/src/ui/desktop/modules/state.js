import { APP_VERSION, fontOptions, llamaDownloadPresets, providerPresets } from './constants.js';
...
export const state = {
  ...
  llamaServerConfig: {
    ...
    modelPreset: 'qwen3-0.6b-q4',
    modelPath: '',
    hfRepo: 'Qwen/Qwen3-0.6B-GGUF:Q4_K_M',
  ...
  uiFontFamily:
    fontOptions.ui.find((option) => option.id === 'system')?.css || fontOptions.ui[0].css,
...
export function toggleChatPanel(panel, open) {
  ...
  if (!nextCollapsed) state.chatControlsOpen = false;
  recordUiEvent(
...
export function toggleChatControls(open) {
  state.chatControlsOpen = typeof open === 'boolean' ? open : !state.chatControlsOpen;
  if (state.chatControlsOpen) {
    ...
    recordUiEvent('chat.core_controls_opened', 'ok', 'Opened Argentum Core controls.', {
    ...
    .map((block) => {
      ...
      const id =
        typeof block.id === 'string' && block.id.trim()
          ? block.id.trim().slice(0, 160)
          : `${Date.now()}-${generateSecureId()}`;
      const parsed =
        role === 'user'
          ? {
              body: block.body,
...
export function hydrateUiPreferences() {
  try {
    ...
    if (saved.llamaServerConfig && typeof saved.llamaServerConfig === 'object') {
      ...
      const savedPreset = llamaDownloadPresets.find(
        (preset) => preset.id === state.llamaServerConfig.modelPreset,
      );
      if (savedPreset) {
        state.llamaServerConfig = {
          ...state.llamaServerConfig,
          modelSource: 'huggingface',
          modelPath: '',
          hfRepo: savedPreset.repo,
          hfFile: savedPreset.file || '',
        };
      }
      if (
...
export function syncLlamaServerModel(modelId = state.providerModel) {
  if (state.llmProvider !== 'llama-cpp') return;
...
export function setProvider(provider) {
  ...
  if (provider.id === 'llama-cpp') {
    state.securityProfile = 'restricted';
    const access = new Set(state.selectedContextAccess || []);
    access.add('local-server');
