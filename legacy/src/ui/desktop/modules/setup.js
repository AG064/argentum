...
function applySavedSetup(savedSetup) {
  ...
  const requestedProvider = String(savedSetup.llmProvider || '').trim();
  const provider =
    providerPresets.find((item) => item.id === requestedProvider) ||
    providerPresets.find((item) => item.id === 'custom');
  if (!provider) return false;

  const authMethods = provider.authMethods || ['api-key'];
  const requestedAuthMethod = String(savedSetup.providerAuthMethod || '').trim();
  const providerAuthMethod = authMethods.includes(requestedAuthMethod)
    ? requestedAuthMethod
    : authMethods[0];
  const selectedChannels = Array.isArray(savedSetup.selectedChannels)
    ? savedSetup.selectedChannels.filter((channel) =>
        ['local', 'webchat', 'telegram', 'whatsapp'].includes(channel),
      )
    : [];
  const selectedContextAccess = Array.isArray(savedSetup.selectedContextAccess)
    ? savedSetup.selectedContextAccess.filter((access) =>
        [
          'workspace-summary',
          'profile',
          'logs',
  ...
  state.workspacePath = String(savedSetup.workspacePath || state.workspacePath).trim();
  state.savedConfigPath = String(savedSetup.configPath || state.savedConfigPath).trim();
  state.experienceLevel = ['beginner', 'comfortable', 'expert'].includes(savedSetup.experienceLevel)
    ? savedSetup.experienceLevel
    : 'beginner';
  state.runtimeMode = ['desktop', 'cli', 'service'].includes(savedSetup.runtimeMode)
    ? savedSetup.runtimeMode
    : 'desktop';
  state.llmProvider = provider.id;
  state.providerApi = ['openai', 'anthropic'].includes(savedSetup.providerApi)
    ? savedSetup.providerApi
    : provider.api;
  state.providerBaseUrl = String(
    savedSetup.providerBaseUrl || provider.defaultBaseUrl || '',
  ).trim();
  state.providerModel = String(savedSetup.providerModel || provider.defaultModel || '').trim();
  state.providerAuthMethod = providerAuthMethod;
  state.providerCatalogTab = provider.access || 'testing';
  state.providerSetupStage = 'model';
  state.providerSelectionConfirmed = true;
  state.customProviderName = String(
    savedSetup.customProviderName || state.customProviderName || 'custom',
  ).trim();
  state.customApiKeyEnv = String(
    savedSetup.providerApiKeyEnv || provider.apiKeyEnv || state.customApiKeyEnv,
  ).trim();
  state.agentName = String(savedSetup.agentName || state.agentName || 'Argentum');
  state.userName = String(savedSetup.userName || '');
  state.systemPrompt = String(savedSetup.systemPrompt || state.systemPrompt);
  state.selectedContextAccess = selectedContextAccess;
  state.thinkingLevel = ['fast', 'balanced', 'deep'].includes(savedSetup.thinkingLevel)
    ? savedSetup.thinkingLevel
    : 'balanced';
  state.showThinkingInChat = Boolean(savedSetup.showThinkingInChat);
  state.showThinkingInTelegram = Boolean(savedSetup.showThinkingInTelegram);
  state.selectedChannels = [...new Set(['local', ...selectedChannels])];
  state.securityProfile = ['restricted', 'ask', 'session', 'trusted'].includes(
    savedSetup.securityProfile,
  )
    ? savedSetup.securityProfile
    : 'restricted';
  if (savedSetup.llamaServer && typeof savedSetup.llamaServer === 'object') {
    state.llamaServerConfig = {
      ...state.llamaServerConfig,
      ...savedSetup.llamaServer,
    };
  }
  ...
  state.apiTest = {
    status: 'idle',
    message: 'Saved provider settings loaded. Test the provider to verify current access.',
  };
  ensureProviderModelAllowed();
  return Boolean(state.workspacePath && state.providerModel);
}
...
export async function hydrateDesktopDefaults() {
  ...
  try {
    ...
    if (applySavedSetup(defaults?.savedSetup)) {
      setUiPreference('workspacePath', state.workspacePath);
      detectMigrationSources();
      recordUiEvent(
        'setup.restore',
        'ok',
        'Saved workspace, provider, model, and security settings were restored.',
        {
          workspacePath: state.workspacePath,
          provider: state.llmProvider,
          model: state.providerModel,
