...
function renderSettingsNav(activeSection) {
  return settingsSections
    .map(
      ([id, label]) => `
        <button class="${activeSection === id ? 'active' : ''}" type="button" data-settings-section="${escapeAttribute(id)}" ${activeSection === id ? 'aria-current="page"' : ''}>
          ${escapeHtml(label)}
        </button>
      `,
    )
...
function settingsSectionSummary(id, state, provider, metadata) {
  const summaries = {
    ...
    'skills': 'Built-in and installed capability packages',
    'feedback': 'Bug reports, private email, and feature requests',
  };
    ...
    const renderSkillCard = (skill) => {
      ...
      return `
        <div class="skill-card">
          ...
          ${
            isArgentumBuiltin
              ? `<p class="skill-builtin-note"><em>Part of the standard Argentum installation and always available.</em></p>`
  ...
  render(state) {
    ...
    const action = isUpdate
      ? state.updateDownloading
        ? '<button class="button primary" type="button" disabled>Opening release page...</button>'
        : `<button class="button primary" id="download-update">Open GitHub release ${escapeHtml(newVersion)}</button>`
      : `<button class="button ${state.updateChecked ? '' : 'primary'}" id="check-for-updates" ${state.updateDownloading ? 'disabled' : ''}>${state.updateDownloading ? 'Checking...' : 'Check for updates'}</button>`;
    ...
    return `
      ...
      ${renderProductTabShell(
        ...
        `
          ${renderCompactPageHeader({
            kicker: 'Updates',
            title: isUpdate ? `Argentum ${newVersion} is available` : 'Keep Argentum current',
            detail: updateStatus,
            tone: isUpdate ? 'warning' : 'standby',
            stats: [
              { label: 'Installed', value: currentVersion },
              { label: 'Latest found', value: state.updateChecked ? newVersion : 'Not checked' },
              { label: 'Source', value: 'GitHub Releases' },
            ],
            actions: action,
          })}
          <div class="update-content-grid">
            <section class="panel update-summary-panel">
              <div class="panel-header">
                <span class="pill ${isUpdate ? 'warn' : state.updateChecked ? 'ok' : ''}">
                  ${isUpdate ? 'Update available' : state.updateChecked ? 'Checked' : 'Not checked'}
                </span>
                <h3>Release status</h3>
                <p>Current version: ${escapeHtml(currentVersion)}</p>
              </div>
              <div class="panel-body update-status-copy">
                ${
                  isUpdate
            ...
            <section class="panel update-policy-panel">
              <div class="panel-header">
                <span class="pill">Recovery</span>
                <h3>Startup release checks</h3>
                <p>The updater remains available even when another interface module cannot load.</p>
              </div>
              <div class="panel-body">
                <p>Argentum checks GitHub Releases on startup. Signed in-place installation is not enabled yet, so the app opens the published release page without claiming it was updated.</p>
              </div>
            </section>
          </div>
        `,
