...
struct DesktopDefaultsResponse {
    ...
    saved_setup: Option<SavedSetupResponse>,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct SavedSetupResponse {
    workspace_path: String,
    config_path: String,
    experience_level: String,
    runtime_mode: String,
...
const LLAMA_SERVER_DEFAULT_HF_REPO: &str = "Qwen/Qwen3-0.6B-GGUF:Q4_K_M";
const LLAMA_SERVER_DEFAULT_HF_FILE: &str = "Qwen3-0.6B-Q4_K_M.gguf";
...
fn llama_hf_preset(id: &str) -> Option<(&'static str, &'static str)> {
    match id {
        ...
        "qwen3-0.6b-q4" => Some(("Qwen/Qwen3-0.6B-GGUF:Q4_K_M", "Qwen3-0.6B-Q4_K_M.gguf")),
        "qwen3.5-0.8b-q4" => Some((
            "ggml-org/Qwen3.5-0.8B-GGUF:Q4_K_M",
        ...
        "gemma-4-e2b-it-q4" => Some((
            "ggml-org/gemma-4-E2B-it-GGUF:Q4_0",
            "gemma-4-E2B-it-Q4_0.gguf",
        )),
        "gemma-4-e4b-it-q4" => Some((
            "ggml-org/gemma-4-E4B-it-GGUF:Q4_0",
            "gemma-4-E4B-it-Q4_0.gguf",
        )),
        "qwen3-coder-30b-q4" => Some((
            "tensorblock/Qwen_Qwen3-Coder-30B-A3B-Instruct-GGUF:Q4_K_M",
            "Qwen3-Coder-30B-A3B-Instruct-Q4_K_M.gguf",
        )),
        "qwen3.6-35b-a3b-q4" => Some(("ggml-org/Qwen3.6-35B-A3B-GGUF:Q4_K_M", "")),
        _ => None,
    }
}
...
fn wait_for_port_or_process_exit(
    ...
    while started.elapsed() < timeout {
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                let recent_output = read_preview(log_path, 80);
                let diagnosis = if recent_output.contains("Could not establish connection")
                    || recent_output.contains("Failed to read connection")
                    || recent_output.contains("HTTPLIB failed")
                {
                    "Model or vision-projector download failed. Check the network connection, proxy, firewall, or download cache."
                } else if recent_output.contains("failed to load model")
                    || recent_output.contains("model loading error")
                {
                    "The selected model was found, but llama.cpp could not load it. Review the model and projector entries in the log."
                } else {
                    "llama.cpp exited during startup. Review the recent log output below."
                };
                return Err(format!(
                    "Argentum llama.cpp server exited before opening port {port} ({status}). {diagnosis} Log: {}. Recent output:\n{}",
                    log_path.display(),
                    recent_output
                ));
            }
...
fn llama_server_previous_log_path(workspace: &Path) -> PathBuf {
    workspace.join("data").join("llama-server.previous.log")
}

fn llama_server_config_path(workspace: &Path) -> PathBuf {
    workspace.join("data").join("llama-server.json")
}

fn read_saved_llama_config(workspace: &Path) -> Option<LlamaServerActionConfig> {
    let contents = std::fs::read_to_string(llama_server_config_path(workspace)).ok()?;
...
fn stop_saved_llama_process(workspace: &Path) -> Result<Option<String>, String> {
    let Some(pid) = read_gateway_pid(&llama_server_pid_path(workspace)) else {
        return Ok(None);
    };
    ...
    if !stopped {
        return Err(format!(
            "Existing llama.cpp process {pid} could not be stopped before reinstalling."
        ));
    }
    let _ = std::fs::remove_file(llama_server_pid_path(workspace));
    Ok(Some(pid))
}
...
fn install_llama_server_binary(workspace: &Path) -> Result<RunDesktopActionResponse, String> {
    ...
    let previous_pid = stop_saved_llama_process(workspace)?;
    ...
    let replacing_install = install_dir.exists();
    if replacing_install {
    ...
    Ok(RunDesktopActionResponse {
        ...
        output: [
            ...
            if replacing_install {
                "Existing managed installation replaced in place.".to_string()
            } else {
                "No existing managed installation was found.".to_string()
            },
            if let Some(pid) = previous_pid {
                format!("Stopped previous llama.cpp process: {pid}")
            } else {
                "No previous llama.cpp process was running.".to_string()
            },
...
fn configured_llama_hf_launch(
    ...
    let repo = preset
        .map(|(repo, _file)| repo.to_string())
        .or(explicit_repo.clone())
    ...
    let file = preset
        ...
        .map(str::to_string)
        .or_else(|| trimmed_config_value(config.and_then(|config| config.hf_file.as_deref())))
        .or_else(|| {
            explicit_repo
                .is_none()
...
fn read_llama_pid(workspace: &Path, port: u16) -> Option<String> {
    ...
    if let Some(pid_value) = pid.as_deref() {
        if port_accepts_connections(port) || process_is_running(pid_value) {
            return pid;
        }
    }
    None
...
fn run_llama_server_action(
    ...
    let saved_config = if config.is_none() {
        read_saved_llama_config(workspace)
    } else {
        None
    };
    let config = config.or(saved_config.as_ref());
    ...
    match action_id {
        ...
        "llama-server-start" => {
            ...
            if log_path.exists() {
                let previous_log_path = llama_server_previous_log_path(workspace);
                let _ = std::fs::remove_file(&previous_log_path);
                std::fs::rename(&log_path, &previous_log_path).map_err(|error| {
                    format!(
                        "Failed to rotate the previous llama.cpp log {}: {error}",
                        log_path.display()
                    )
                })?;
            }
            ...
            writeln!(
                &log_file,
                "\n[Argentum] starting llama.cpp pid= pending endpoint={} model={} args={:?}",
                endpoint, model_label, args
            )
            .map_err(|error| format!("Failed to write llama.cpp startup log: {error}"))?;
            ...
            let port_ready =
                wait_for_port_or_process_exit(&mut child, port, Duration::from_secs(2), &log_path)?;
...
fn saved_setup_from_workspace(workspace: &Path) -> Option<SavedSetupResponse> {
    let config_path = workspace.join("config").join("default.yaml");
    let contents = std::fs::read_to_string(&config_path).ok()?;
    let yaml = serde_yaml::from_str::<serde_yaml::Value>(&contents).ok()?;
    let config = provider_runtime_config(workspace).ok()?;
    ...
    let llm_provider = if known_provider {
        config.name.clone()
    } else {
        "custom".to_string()
    };
    let custom_provider_name = if known_provider {
        String::new()
    } else {
        config.name.clone()
    };

    Some(SavedSetupResponse {
        workspace_path: workspace.display().to_string(),
        config_path: config_path.display().to_string(),
        experience_level: yaml_string_at(&yaml, &["experienceLevel"])
            .unwrap_or("beginner")
            .to_string(),
        ...
        provider_auth_method: config.auth_method,
        provider_api_key_env: config.api_key_env,
        custom_provider_name,
        agent_name: config.agent_name,
        user_name: config.user_name,
        system_prompt: config.system_prompt,
        selected_context_access: config.selected_context_access,
        thinking_level: config.thinking_level,
        show_thinking_in_chat: yaml_bool_at(&yaml, &["profile", "reasoningOutput", "chat"]),
        show_thinking_in_telegram: yaml_bool_at(&yaml, &["profile", "reasoningOutput", "telegram"]),
        selected_channels: config.selected_channels,
        security_profile: config.security_profile,
        llama_server: read_saved_llama_config(workspace),
    })
}
...
fn desktop_defaults() -> DesktopDefaultsResponse {
    let saved_workspace = read_saved_workspace_path();
    let saved_setup = saved_workspace
        .as_deref()
        .and_then(saved_setup_from_workspace);

    DesktopDefaultsResponse {
        default_workspace_path: default_workspace_path().display().to_string(),
        saved_workspace_path: saved_workspace.map(|path| path.display().to_string()),
        saved_setup,
    }
}
...
fn desktop_state(
    ...
    let logs_dir = workspace.join("logs");
    let gateway_pid_path = data_dir.join(".gateway.pid");
    let saved_llama_config = if request.llama_server.is_none() {
        read_saved_llama_config(&workspace)
    } else {
        None
    };
    let llama_config = request
        .llama_server
        .as_ref()
        .or(saved_llama_config.as_ref());
    let llama_port = llama_server_port(llama_config);
...
fn save_setup(request: SaveSetupRequest) -> Result<SaveSetupResponse, String> {
    ...
    if let Some(llama_server) = request.llama_server.as_ref() {
        let llama_config = serde_json::to_string_pretty(llama_server)
            .map_err(|error| format!("Failed to serialize llama.cpp settings: {error}"))?;
        write_text(&llama_server_config_path(&workspace), &llama_config)?;
    }
    write_text(
        &secrets_path,
...
mod tests {
    ...
    #[test]
    fn saved_setup_restores_selected_provider_and_model() {
        let workspace = std::env::temp_dir().join(format!(
            "argentum-saved-setup-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("clock should work")
...
llm:
  ...
  providers:
    llama-cpp:
      label: "Argentum llama.cpp"
      base_url: "http://127.0.0.1:8080/v1"
      api_key_env: "ARGENTUM_LLAMA_API_KEY"
      api: openai
      auth_method: api-key
      models:
...
"#,
        ...
        let restored = saved_setup_from_workspace(&workspace).expect("saved setup should load");
        assert_eq!(restored.llm_provider, "llama-cpp");
        assert_eq!(restored.provider_model, "google/gemma-4-e2b-it");
        assert_eq!(restored.user_name, "Test User");
        assert_eq!(restored.thinking_level, "deep");
        assert!(restored.show_thinking_in_chat);
        assert_eq!(restored.selected_channels, ["local", "telegram"]);
        assert!(restored
            .selected_context_access
            .contains(&"local-server".to_string()));
