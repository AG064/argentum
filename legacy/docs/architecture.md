...
> This document describes the current v0.0.9 TypeScript and Node-based
> framework architecture. The target architecture for the full Rust rewrite and
> premium agent harness is defined in
> [Premium Agent Harness Plan](PREMIUM_AGENT_HARNESS_PLAN.md). The current
> architecture remains reference material until each Rust replacement passes
> its migration and release gates.
