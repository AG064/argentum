...
class GoalDecompositionFeature {
    ...
    createGoal(title, description) {
        ...
        this.db.prepare(`INSERT INTO tasks (id, goal_id, parent_id, title, description, status, priority, created_at, updated_at)
       VALUES (?, ?, NULL, ?, ?, 'pending', 5, ?, ?)`).run(id, id, title, description ?? '', now, now);
    ...
    addSubTask(goalId, title, description, parentId, priority = 5) {
        ...
        this.db.prepare(`INSERT INTO tasks (id, goal_id, parent_id, title, description, status, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?)`).run(id, goalId, parentId ?? null, title, description ?? '', priority, now, now);
    ...
    getTaskTree(goalId) {
        const root = this.db.prepare('SELECT * FROM tasks WHERE id = ? AND goal_id = ?').get(goalId, goalId);
        if (!root)
            return null;
        const all = this.db.prepare('SELECT * FROM tasks WHERE goal_id = ? ORDER BY priority DESC, created_at ASC').all(goalId);
        const map = {};
        for (const t of all)
            map[t.id] = { ...t, children: [] };
        for (const t of all) {
