...
class ContentFilteringFeature implements FeatureModule {
  ...
  private config: Required<ContentFilteringConfig>;
   ...
   */
  check(text: string, filter: boolean = true): ContentCheckResult {
    const violations: ContentCheckResult['violations'] = [];
    let filteredText = text;
    let offset = 0;

    // Check enabled built-in rules
    for (const rule of this.builtInRules) {
      if (!this.config.enabledRuleTypes.includes(rule.type)) continue;
  ...
  private replaceAt(str: string, pos: number, len: number, replacement: string): string {
    return str.substring(0, pos) + replacement + str.substring(pos + len);
  }
}

export default new ContentFilteringFeature();
