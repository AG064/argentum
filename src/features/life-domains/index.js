...
class LifeDomainsFeature {
    ...
    createDomain(name, description, options) {
        ...
        this.db.prepare(`INSERT INTO domains (id, name, description, color, icon, parent_id, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, name, description, domain.color, domain.icon, domain.parentId, domain.priority, now, now);
    ...
    updateDomain(id, updates) {
        ...
        this.db.prepare('UPDATE domains SET name=?, description=?, color=?, icon=?, priority=?, updated_at=? WHERE id=?').run(merged.name, merged.description, merged.color, merged.icon, merged.priority, merged.updatedAt, id);
        return merged;
    }
    ...
    addEntry(domainId, type, title, content, options) {
        ...
        this.db.prepare(`INSERT INTO domain_entries (id, domain_id, type, title, content, tags, importance, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, domainId, type, title, content, JSON.stringify(entry.tags), entry.importance, JSON.stringify(entry.metadata), now, now);
    ...
    updateEntry(id, updates) {
        ...
        if (!existing)
            return null;
        const merged = { ...existing, ...updates, updatedAt: Date.now() };
        this.db.prepare('UPDATE domain_entries SET title=?, content=?, tags=?, importance=?, metadata=?, updated_at=? WHERE id=?').run(merged.title, merged.content, JSON.stringify(merged.tags), merged.importance, JSON.stringify(merged.metadata), merged.updatedAt, id);
        return merged;
    }
    /** Delete an entry */
    deleteEntry(id) {
    ...
    seedDefaultDomains() {
        const stmt = this.db.prepare(`INSERT INTO domains (id, name, description, color, icon, parent_id, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, NULL, 5, ?, ?)`);
        const now = Date.now();
        for (const domain of this.config.defaultDomains) {
            stmt.run((0, crypto_1.randomUUID)(), domain.name, domain.description, domain.color, domain.icon, now, now);
        }
