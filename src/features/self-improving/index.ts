...
class SelfImprovingLoop implements FeatureModule {
  ...
  private config: SelfImprovingConfig = { ...DEFAULT_CONFIG };
  ...
  async healthCheck(): Promise<HealthStatus> {
    ...
    const lessonsCount = existsSync(lessonsPath)
   ...
   */
  async run(
    phase: 'all' | 'error' | 'skill' | 'memory' | 'model' | 'correction' = 'all',
    ...
    if (options.force !== undefined) this.config.forceRun = options.force;
    if (options.verbose !== undefined) this.config.verbose = options.verbose;
   ...
   */
  async dryRun(): Promise<SelfImprovingResult> {
  ...
  private async runErrorAnalysis(): Promise<PhaseResult> {
  ...
  private async runSkillCreation(): Promise<PhaseResult> {
    const start = Date.now();
    const phase: PhaseResult = {
      phase: 'skill',
      success: false,
  ...
  private async runMemoryConsolidation(): Promise<PhaseResult> {
    const start = Date.now();
    const phase: PhaseResult = {
      phase: 'memory',
      success: false,
  ...
  private async runUserModelUpdate(): Promise<PhaseResult> {
    const start = Date.now();
    const phase: PhaseResult = {
      phase: 'model',
      success: false,
  ...
  private async runSelfCorrection(): Promise<PhaseResult> {
    const start = Date.now();
    const phase: PhaseResult = {
      phase: 'correction',
      success: false,
  ...
  private log(msg: string, opts: { error?: boolean; phase?: string; dryRun?: boolean } = {}): void {
    if (opts.error) {
  ...
  private logResult(result: SelfImprovingResult): void {
  ...
  private gatherSessionContent(): Promise<string[]> {
  ...
  private findRecentMemoryFiles(): string[] {
    const memoryFiles: string[] = [];
    const memoryBase = path.join(this.memoryDir);

    if (!existsSync(memoryBase)) return memoryFiles;
  ...
  private async extractInsights(files: string[]): Promise<string[]> {
  ...
  private mergeIntoMemory(insights: string[]): void {
  ...
  private async analyzeUserPatterns(): Promise<string[]> {
  ...
  private updateUserModelingMd(patterns: string[]): void {
  ...
  private patternToArea(pattern: string): string {
