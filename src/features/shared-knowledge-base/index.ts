...
class SharedKnowledgeBaseFeature implements FeatureModule {
  ...
  async init(config: Record<string, unknown>, context: FeatureContext): Promise<void> {
    this.ctx = context;
    this.config = { ...this.config, ...(config as Partial<KnowledgeBaseConfig>) };
    this.initDatabase();
  }
  ...
  async search(query: string, limit: number = 20, offset: number = 0): Promise<Article[]> {
    if (!query.trim()) {
      return [];
    }
...
export default new SharedKnowledgeBaseFeature();
