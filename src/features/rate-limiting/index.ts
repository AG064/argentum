...
class RateLimitingFeature implements FeatureModule {
  ...
  private checkInternal(key: string, limit: number, windowMs: number): { allowed: boolean; count: number } {
    const now = Date.now();
    const cutoff = now - windowMs;

    // Clean up old entries occasionally (every 1000th check)
    if (Math.random() < 0.001) {
      this.db.prepare('DELETE FROM rate_windows WHERE timestamp < ?').run(Date.now() - windowMs * 2);
    }

    const insert = this.db.prepare('INSERT INTO rate_windows (key, timestamp) VALUES (?, ?)');
    insert.run(key, now);

    const row = this.db.prepare('SELECT COUNT(*) as c FROM rate_windows WHERE key = ? AND timestamp >= ?').get(key, cutoff) as { c: number };
    const count = row.c;
  ...
  getStats(key: string): number[] {
    const rows = this.db.prepare('SELECT timestamp FROM rate_windows WHERE key = ? ORDER BY timestamp DESC LIMIT 1000').all(key) as Array<{ timestamp: number }>;
