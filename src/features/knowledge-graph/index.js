...
class SQLiteGraphBackend {
    ...
    async addEntity(data) {
        ...
        const now = Date.now();
        this.db.prepare('INSERT INTO entities (id, type, name, properties, tags, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, data.type, data.name, JSON.stringify(data.properties), JSON.stringify(data.tags ?? []), now, now);
    ...
    async updateEntity(id, updates) {
        ...
        this.db.prepare('UPDATE entities SET name=?, properties=?, tags=?, updated_at=? WHERE id=?').run(merged.name, JSON.stringify(merged.properties), JSON.stringify(merged.tags), now, id);
        return merged;
    }
    ...
    async addRelationship(data) {
        ...
        const now = Date.now();
        this.db.prepare('INSERT INTO relationships (id, source_id, target_id, type, properties, weight, created_at) VALUES (?,?,?,?,?,?,?)').run(id, data.sourceId, data.targetId, data.type, JSON.stringify(data.properties), data.weight, now);
    ...
    rowToRel(row) {
        return {
            ...
            type: row.type, properties: JSON.parse(row.properties),
...
class MemoryGraphBackend {
    ...
    async getAllRelationships() { return Array.from(this.relationships.values()); }
    async getAllEntities() { return Array.from(this.entities.values()); }
    async findEntities(query) {
    ...
    async clear() { this.entities.clear(); this.relationships.clear(); }
}
// ─── Main Feature ────────────────────────────────────────────────────────────
/**
...
class KnowledgeGraphFeature {
    ...
    async stop() { await this.backend?.close(); this.backend = null; }
    async healthCheck() {
        if (!this.backend)
