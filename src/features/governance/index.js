...
class GovernanceFeature {
    ...
    async approve(ticketId, approver) {
        ...
        if (ticket.status !== 'pending')
            throw new Error(`Ticket is not pending: ${ticket.status}`);
        if (this.config.approvers.length > 0 && !this.config.approvers.includes(approver)) {
            throw new Error(`Unauthorized approver: ${approver}`);
        }
        const now = Date.now();
        this.db.prepare('UPDATE approval_tickets SET status = ?, approver = ?, resolved_at = ? WHERE id = ?').run('approved', approver, now, ticketId);
    ...
    async reject(ticketId, approver, reason) {
        ...
        if (ticket.status !== 'pending')
            throw new Error(`Ticket is not pending: ${ticket.status}`);
        const now = Date.now();
        this.db.prepare('UPDATE approval_tickets SET status = ?, approver = ?, reason = ?, resolved_at = ? WHERE id = ?').run('rejected', approver, reason, now, ticketId);
    ...
    async rollback(configId) {
        const ticket = this.db.prepare("SELECT * FROM approval_tickets WHERE id = ? AND status = 'approved'").get(configId);
        if (!ticket) {
    ...
    async listTickets(status) {
        const rows = status
            ? this.db.prepare('SELECT * FROM approval_tickets WHERE status = ? ORDER BY created_at DESC').all(status)
    ...
    createTicket(action, requester, status, reason) {
        ...
        this.db.prepare(`INSERT INTO approval_tickets
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, action.type, action.description, JSON.stringify(action.payload), action.risk, requester, status, status === 'approved' ? 'system' : null, reason ?? null, now, status === 'approved' ? now : null, expiresAt);
        if (status === 'pending') {
    ...
    getPendingSync() {
        const rows = this.db.prepare("SELECT * FROM approval_tickets WHERE status = 'pending' ORDER BY created_at ASC").all();
        return rows.map(this.rowToTicket);
    }
    getExpiredCount() {
        const now = Date.now();
        const row = this.db.prepare("SELECT COUNT(*) as c FROM approval_tickets WHERE status = 'pending' AND expires_at <= ?").get(now);
        return row.c;
    }
    expireOldTickets() {
        const now = Date.now();
        const result = this.db.prepare("UPDATE approval_tickets SET status = 'expired', resolved_at = ? WHERE status = 'pending' AND expires_at <= ?").run(now, now);
        if (result.changes > 0) {
