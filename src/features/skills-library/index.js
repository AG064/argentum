...
class SkillsLibraryFeature {
    ...
    registerSkill(payload) {
        ...
        this.db.prepare(`INSERT INTO skills (id, name, version, description, code, tags, author, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, payload.name, version, payload.description ?? '', payload.code, JSON.stringify(payload.tags ?? []), payload.author ?? '', now, now);
        const rec = {
            id,
            name: payload.name,
            version,
    ...
    searchSkills(query) {
        ...
        const rows = this.db.prepare(`SELECT * FROM skills WHERE LOWER(name) LIKE ? OR LOWER(description) LIKE ? OR LOWER(tags) LIKE ? ORDER BY updated_at DESC LIMIT 100`).all(q, q, q);
        return rows.map(r => this.rowToSkill(r));
    }
    updateSkill(id, patch) {
        ...
        this.db.prepare(`UPDATE skills SET name = ?, version = ?, description = ?, code = ?, tags = ?, author = ?, updated_at = ? WHERE id = ?`).run(updated.name, updated.version, updated.description, updated.code, JSON.stringify(updated.tags), updated.author ?? '', updated.updatedAt, id);
