...
class BrowserAutomationFeature {
    ...
    async launchBrowser(proxyUrl) {
        ...
        if (!this.engine)
            throw new Error('Browser engine not installed');
    ...
    async executeAction(sessionId, action) {
        ...
        if (!page)
            return { success: true, data: { note: 'No browser engine, action logged only' } };
        try {
            switch (action.type) {
                case 'click':
                ...
                case 'extract': {
                    if (action.extractAll) {
                        const data = await page.$$eval(action.selector, (els, attr) => els.map(el => attr ? el.getAttribute(attr) : el.textContent?.trim()), action.attribute);
                        return { success: true, data };
                    }
                    const data = action.attribute
                        ? await page.$eval(action.selector, (el, attr) => el.getAttribute(attr), action.attribute)
            ...
            return await page.evaluate(() => {
                ...
                return {
                    ...
                    links: Array.from(document.querySelectorAll('a[href]')).map(a => ({
                        text: getText(a), href: a.href,
                    })).filter(l => l.href.startsWith('http')).slice(0, 200),
                    images: Array.from(document.querySelectorAll('img[src]')).map(img => ({
                        src: img.src, alt: img.alt,
                    })).slice(0, 100),
                    meta: Object.fromEntries(Array.from(document.querySelectorAll('meta[name], meta[property]')).map(m => [
                        m.name || m.getAttribute('property') || '',
                        m.content,
                    ]).filter(([k]) => k)),
                };
            });
