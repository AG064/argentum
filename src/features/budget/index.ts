...
class BudgetFeature implements FeatureModule {
  ...
  getBudgetReport(): BudgetReport {
    ...
    for (const status of byAgent) {
      if (status.percentUsed >= this.config.alertThreshold * 100) {
        alerts.push(
          `${status.agent}: ${status.percentUsed}% of limit ($${status.totalCost.toFixed(4)})`,
        );
      }
    }
  ...
  private initDatabase(): void {
    ...
    if (savedConfig) {
      try {
        const parsed = JSON.parse(savedConfig.value) as BudgetConfig;
  ...
  private getAgentDailyUsage(
    ...
    const row = this.db
      ...
      .get(agentId, dayStart) as { tokens: number; cost: number };
  ...
  private getAgentMonthlyUsage(
    ...
    const row = this.db
      ...
      .get(agentId, monthStart) as { tokens: number; cost: number };
    return { totalTokens: row.tokens, totalCost: row.cost };
  }

  private getDailyUsage(): { totalTokens: number; totalCost: number } {
    const dayStart = this.getDayStart();
    const row = this.db
      .prepare(
        `SELECT
      ...
      .get(dayStart) as { tokens: number; cost: number };
    return { totalTokens: row.tokens, totalCost: row.cost };
  }
  ...
  private getMonthlyUsage(): { totalTokens: number; totalCost: number } {
    ...
    const row = this.db
      ...
      .get(monthStart) as { tokens: number; cost: number };
    return { totalTokens: row.tokens, totalCost: row.cost };
  }
  ...
  private checkAndAlert(agentId: string): void {
    ...
    if (
      status.percentUsed >= this.config.alertThreshold * 100 &&
