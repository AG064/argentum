...
class AutoCaptureFeature {
  ...
  extractText(data) {
    ...
    if (typeof data === 'object' && data !== null) {
      const obj = data;
      return obj.text ?? obj.content ?? obj.message ?? null;
    }
    return null;
  }
  /** Detect captures from text */
  detectCaptures(text, source) {
    const captures = [];
    const seen = new Set();
  ...
  async saveCapture(capture) {
    try {
      ...
      const graph = (0, graph_1.getMemoryGraph)();
  ...
  async analyzeText(text, source = 'manual') {
...
exports.default = new AutoCaptureFeature();
