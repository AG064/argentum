...
class TaskCheckoutFeature {
    ...
    async checkout(taskId, agentId) {
        ...
        const agentLeases = this.db.prepare('SELECT COUNT(*) as c FROM checkouts WHERE agent_id = ?').get(agentId);
        ...
        try {
            const stmt = this.db.prepare(`INSERT INTO checkouts (task_id, agent_id, checked_out_at, expires_at)
         VALUES (?, ?, ?, ?)`);
            ...
            if (e.code === 'SQLITE_CONSTRAINT_UNIQUE' || e.message?.includes('UNIQUE')) {
                ...
                if (existing && existing.expires_at < now) {
                    ...
                    this.db.prepare('UPDATE checkouts SET agent_id = ?, checked_out_at = ?, expires_at = ? WHERE task_id = ?').run(agentId, now, expiresAt, taskId);
    ...
    async getAgentTasks(agentId) {
        const rows = this.db.prepare('SELECT * FROM checkouts WHERE agent_id = ? ORDER BY checked_out_at ASC').all(agentId);
        return rows.map(r => ({
            taskId: r.task_id,
            agentId: r.agent_id,
    ...
    async extendLease(taskId) {
        ...
        const result = this.db.prepare('UPDATE checkouts SET expires_at = ? WHERE task_id = ?').run(newExpiry, taskId);
    ...
    getCheckedOutTasksSync() {
        const now = Date.now();
        const rows = this.db.prepare('SELECT * FROM checkouts WHERE expires_at > ? ORDER BY checked_out_at ASC').all(now);
        return rows.map(r => ({
            taskId: r.task_id,
            agentId: r.agent_id,
            checkedOutAt: r.checked_out_at,
    ...
    getExpiredCount() {
        const now = Date.now();
        const row = this.db.prepare('SELECT COUNT(*) as c FROM checkouts WHERE expires_at <= ?').get(now);
        return row.c;
    }
    releaseExpired() {
        const now = Date.now();
