...
class GoalsFeature {
    ...
    async healthCheck() {
        ...
        const active = this.db.prepare("SELECT COUNT(*) as c FROM goals WHERE status = 'active'").get();
        return {
            healthy: true,
    ...
    async createGoal(title, description, parentId) {
        ...
        this.db.prepare(`INSERT INTO goals (id, title, description, parent_id, status, metrics, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, title, description, parentId ?? null, 'active', '{}', now, now);
    ...
    async linkTaskToGoal(taskId, goalId) {
        ...
        const now = Date.now();
        this.db.prepare(`INSERT OR REPLACE INTO task_goals (task_id, goal_id, linked_at) VALUES (?, ?, ?)`).run(taskId, goalId, now);
    ...
    async getGoalProgress(goalId) {
        ...
        if (!row)
            throw new Error(`Goal not found: ${goalId}`);
        const taskCounts = this.db.prepare(`SELECT
        COUNT(*) as total,
        SUM(CASE WHEN t.status = 'completed' THEN 1 ELSE 0 END) as completed
       FROM task_goals tg
       LEFT JOIN tasks t ON t.id = tg.task_id
       WHERE tg.goal_id = ?`).get(goalId);
    ...
    async updateGoalStatus(goalId, status) {
        const result = this.db.prepare('UPDATE goals SET status = ?, updated_at = ? WHERE id = ?').run(status, Date.now(), goalId);
        if (result.changes === 0)
            throw new Error(`Goal not found: ${goalId}`);
    ...
    async listGoals(status) {
        const rows = status
            ? this.db.prepare('SELECT * FROM goals WHERE status = ? ORDER BY created_at DESC').all(status)
    ...
    getTaskCounts() {
        const rows = this.db.prepare(`SELECT tg.goal_id,
        COUNT(*) as total,
        SUM(CASE WHEN t.status = 'completed' THEN 1 ELSE 0 END) as completed
       FROM task_goals tg
       LEFT JOIN tasks t ON t.id = tg.task_id
       GROUP BY tg.goal_id`).all();
        const map = new Map();
        for (const row of rows) {
