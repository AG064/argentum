...
function assertExpression(value: unknown): jsep.Expression {
  if (!value || typeof value !== 'object' || !('type' in value)) {
    throw new Error('Invalid expression node');
  }
  return value as jsep.Expression;
}

function propertyKey(value: unknown): string | number | undefined {
  return typeof value === 'string' || typeof value === 'number' ? value : undefined;
}

function asRecord(value: unknown): Record<string | number, unknown> | undefined {
...
function evalNode(node: jsep.Expression, vars: Record<string, unknown>): unknown {
  switch (node.type) {
    case 'BinaryExpression': {
      ...
      if (binaryNode.operator === '&&') return Boolean(left) && Boolean(right);
      if (binaryNode.operator === '||') return Boolean(left) || Boolean(right);
    ...
    case 'UnaryExpression': {
      ...
      if (unaryNode.operator === '!') return !val;
      if (unaryNode.operator === '+') return +Number(val);
      if (unaryNode.operator === '-') return -Number(val);
    ...
    case 'MemberExpression': {
      ...
      const prop = propertyKey(
        memberNode.computed
          ...
          : (memberNode.property as jsep.Identifier).name,
      );
      return obj && prop !== undefined ? obj[prop] : undefined;
    }
    case 'Compound':
      // Handle multiple expressions (e.g., from jsep parsing)
      if (Array.isArray((node as jsep.Compound).body)) {
        return (node as jsep.Compound).body.reduce(
          (_acc: unknown, expression: jsep.Expression) => evalNode(expression, vars),
          undefined,
        );
      }
      return evalNode(assertExpression((node as { body?: unknown }).body), vars);
    default:
...
class MeshWorkflowsFeature implements FeatureModule {
  ...
  async resumeExecution(executionId: string): Promise<boolean> {
    ...
    if (exec?.status === 'paused') {
      ...
      if (workflow) {
        void this.runExecution(exec, workflow).catch((error: unknown) => {
          exec.status = 'failed';
