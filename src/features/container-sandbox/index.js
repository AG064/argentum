...
const DEFAULT_CONFIG = {
  ...
  cpuLimit: '1.0',
  timeoutMs: 30000,
  workspacePath: (0, path_1.resolve)(process.cwd(), 'data/workspace'),
  networkAccess: false,
  readOnlyRoot: true,
  tmpfsSize: '64m',
};
 ...
 */
function runInSandbox(command, options = {}) {
  ...
  return new Promise((resolvePromise) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      if (containerName) {
        (0, child_process_1.spawn)('docker', ['kill', containerName], { stdio: 'ignore' });
      }
      proc.kill('SIGKILL');
    }, config.timeoutMs);
 ...
 */
function buildDockerArgs(config, command) {
  ...
  const ws = (0, path_1.resolve)(config.workspacePath);
...
class ContainerSandboxFeature {
   ...
   */
  async execute(command, overrides) {
    return runInSandbox(command, { ...this.config, ...overrides });
  }
}
exports.default = new ContainerSandboxFeature();
