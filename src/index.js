...
function createBuiltinTools() {
  return [
    ...
    },
    {
      ...
      execute: async (params) => {
        const { readFileSync, existsSync } = await Promise.resolve().then(() =>
    ...
    },
    {
      ...
      execute: async (params) => {
        const { writeFileSync, mkdirSync } = await Promise.resolve().then(() =>
          __importStar(require('fs')),
        );
  ...
  async startTelegram(token, channelConfig) {
    try {
      ...
      bot.on('message:text', async (ctx) => {
        ...
        try {
          const autoCapture = (
            await Promise.resolve().then(() => __importStar(require('./features/auto-capture')))
          ).default;
