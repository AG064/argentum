    ...
    return [
        {
            ...
            execute: async (params) => {
                ...
                try {
                    ...
                    const related = (data.RelatedTopics ?? []).slice(0, 5).map(t => t.Text).join('\n');
                    if (abstract) {
...
class AGClaw {
    ...
    async start() {
        ...
        try {
            ...
            this.llmProvider = (0, llm_provider_1.createLLMProvider)({
                ...
                fallbackModels: modelConfig?.fallbackModel ? [modelConfig.fallbackModel] : [],
            });
    ...
    async startChannels() {
        ...
        if (channels?.telegram?.enabled !== false) {
            const token = channels?.telegram?.token ?? process.env.AGCLAW_TELEGRAM_TOKEN ?? process.env.TELEGRAM_BOT_TOKEN;
            if (token) {
                try {
    ...
    registerMemoryTools() {
        ...
        this.agent.registerTool({
            ...
            execute: async (params) => {
                ...
                return results.map(r => `[${r.type}] ${r.content.slice(0, 200)} (accessed ${r.access_count}x)`).join('\n');
            },
        });
        this.agent.registerTool({
            name: 'memory_store',
