...
function globMatch(pattern: string, value: string): boolean {
  ...
  // Exact match
  if (normPattern === normValue) return true;
  ...
  if (normPattern.includes('**')) {
    ...
    if (prefix) {
      if (!normValue.startsWith(prefix)) return false;
    }
    if (suffix) {
      if (!normValue.endsWith(suffix) && !normValue.includes(suffix.slice(1))) return false;
    }
    return true;
  }
...
// ─── Approval Store (in-memory + SQLite) ─────────────────────────────────────
...
function approvalToSQL(ar: ApprovalRequest): Record<string, unknown> {
  return {
    id: ar.id,
    agent_id: ar.agentId,
...
function approvalFromSQL(row: Record<string, unknown>): ApprovalRequest {
  return {
    id: row.id as string,
    agentId: row.agent_id as string,
    agentName: (row.agent_name as string) || undefined,
    action: {
      id: (row.action_metadata as string) || randomUUID(),
      agentId: row.agent_id as string,
      type: (row.action_type as AgentAction['type']) || 'read',
      resource: (row.action_resource as AgentAction['resource']) || 'file://unknown',
      metadata: JSON.parse((row.action_metadata as string) || '{}'),
      timestamp: row.requested_at as number,
    },
    risk: (row.risk as ApprovalRisk) || 'medium',
    details: {
      what: (row.details_what as string) || '',
      why: (row.details_why as string) || '',
      consequences: (row.details_consequences as string) || '',
      alternatives: JSON.parse((row.details_alternatives as string) || '[]'),
    },
    status: (row.status as ApprovalRequest['status']) || 'pending',
    requestedAt: row.requested_at as number,
    expiresAt: row.expires_at as number,
    respondedAt: row.responded_at as number | undefined,
...
export class PolicyEngine {
  ...
  private approvalExpirySeconds = 300; // 5 min default
  ...
  private savePolicyToDB(policy: Policy): void {
    if (!this.db) return;
  ...
  evaluate(action: AgentAction): PolicyDecision {
    ...
    for (const policy of sortedPolicies) {
      ...
      if (!resourceMatches(policy.resource, action.resource)) continue;

      // Check action match
      if (policy.action !== '*' && policy.action !== action.type) continue;
  ...
  private createApprovalRequest(
    ...
    if (this.db) {
      ...
      stmt.run(
        sqlData.id,
        sqlData.agent_id,
        sqlData.agent_name ?? null,
        sqlData.action_type,
        sqlData.action_resource,
        sqlData.action_metadata,
        sqlData.risk,
        sqlData.details_what,
        sqlData.details_why,
        sqlData.details_consequences,
        sqlData.details_alternatives,
        sqlData.status,
        sqlData.requested_at,
        sqlData.expires_at,
      );
    }
  ...
  private updateApprovalInDB(request: ApprovalRequest): void {
    if (!this.db) return;
  ...
  logAudit(entry: Omit<AuditEntry, 'id' | 'timestamp' | 'isoTime'>): void {
    ...
    if (this.db) {
      ...
      stmt.run(
        ...
        fullEntry.action,
        fullEntry.severity,
        fullEntry.actor ?? null,
        fullEntry.resource ?? null,
        JSON.stringify(fullEntry.details),
        fullEntry.decision ?? null,
        fullEntry.policyId ?? null,
        fullEntry.approvalId ?? null,
        fullEntry.sessionId ?? null,
        fullEntry.ip ?? null,
        fullEntry.success ? 1 : 0,
        fullEntry.durationMs ?? null,
      );
    }
  ...
  queryAudit(filter?: {
    ...
    return rows.map((row) => ({
      ...
      decision: row.decision as AuditEntry['decision'] | undefined,
  ...
  getStats(): SecurityStats {
    ...
    return {
      ...
      auditEntriesTotal: this.db
        ? (this.db.prepare('SELECT COUNT(*) as c FROM security_audit').get() as { c: number }).c
        : 0,
